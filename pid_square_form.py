...
####randomized initial position####
    ...
    #### Lists for collision values ####
    collision_time_pts = []
    collision_term_values = []
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for m in range(num_drones):
            for n in range(m+1, num_drones):
                ...
                #     collision_avoidance_term = abs(actual_dist**2 - desired_dist**2)**2
                #     total_error += collision_avoidance_term
                #     collision_term_values.append(collision_avoidance_term)
                #     collision_time_pts.append(current_time)
                if actual_dist < SAFE_DISTANCE:
                    collision_avoidance_term += abs(actual_dist ** 2 - desired_dist ** 2)**2
                    # total_error += collision_avoidance_term
                count += 1
    ...
    plt.figure()
    plt.plot(time_pts_error, objective_error_values, label="Objective Function Error")

    plt.legend()
    plt.xlabel('Time (s)')
    plt.ylabel('Objective Function Error')
    plt.title('Objective Function Error Over Time')
    plt.grid(True)

    return time_pts_error, objective_error_values, collision_time_pts, collision_term_values
...
if __name__ == "__main__":
    ...
    time_with, error_with, collision_time_with, collision_values_with = run(**vars(ARGS))

    # Run simulation without collision term
    time_without, error_without, collision_time_without, collision_values_without = run_without_collision_term(**vars(ARGS))
    ...
    # New plot for the collision avoidance term
    plt.figure()
    plt.plot(collision_time_with, collision_values_with, label="With Collision Term")
    # This part plots a horizontal line at 0 for the case without the collision term
    plt.plot(collision_time_without, collision_values_without, label="Without Collision Term")
